...
export const Advantages = ({ data = [] }) => {
  const AdvantageItem = data.map(
    ({ image, title, content, customClass = null }, index) => (
      <div
        className={`advantage-item col-md-4 d-flex flex-column align-items-center flex-row ${customClass}`}
        key={index}
      >
